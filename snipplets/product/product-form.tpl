     ...
     <form id="product_form" class="js-product-form mt-4" method="post" action="{{ store.cart_url }}" data-store="product-form-{{ product.id }}">
        ...
        {% if is_kit %}
            {{ component('kit-products', {
                ...
                select_icon_svg_id: 'chevron',
                kit_products_classes: {
                    container: 'mb-4',
                    list: 'list-unstyled mb-0',
                    item_wrap: 'top-line py-3',
                    ...
                    variants: {
                        container: 'kit-product-variants mt-3',
                        group: 'form-group mb-0',
                        select_wrap: 'form-group mb-0',
                        label: 'form-label',
                        select: 'form-select',
                        select_icon: 'form-select-icon',
                        select_icon_svg: 'icon-inline icon-w-14 icon-rotate-90',
                        ...
                        swatch_image: 'absolute-centered-vertically',
                    },
                },
            }) }}
        ...
        <div class="row {% if settings.product_stock %}mb-3{% else %}mb-4{% endif %}">
            ...
            <div class="js-buy-button-container {{ btn_container_classes }} {% if product.isSubscribable() %}mt-1{% endif %}">
                ...
                {% if is_kit and not store.is_catalog %}
                    {% set kit_buyable_state = state == 'contact' ? 'contact' : 'cart' %}
                    data-kit-label-buyable="{{ texts[kit_buyable_state] | translate }}"
                    data-kit-class-buyable="{{ kit_buyable_state }}"
